...
import cn.niuyannet.kaochang.android.ui.BannerActivity
...
class PlayAdapter(private val mContext: Context, private val dataList: ArrayList<PlayResourcesBean>,
                  private val mActivity: BannerActivity
):
        ...
        holder.video.apply {
            ...
            setVideoAllCallBack(object : GSYSampleCallBack(){
                override fun onComplete(url: String?, vararg objects: Any?) {
                    ...
                    mActivity.binding.banner.isAutoLoop(true)
                    mActivity.binding.banner.start()
                }
                ...
                override fun onAutoComplete(url: String?, vararg objects: Any?) {
                    ...
                    // 检查是否只有一个视频，如果是则不进行轮播
                    if (dataList.size <= 1 && !dataList[0].isImg) {
                        // 只有一个视频，播放完成后重新播放
                        holder.video.startPlayLogic()
                    } else {
                        mActivity.binding.banner.isAutoLoop(true)
                        //快速轮播到下一页
                        mActivity.binding.banner.setLoopTime(100)
                        mActivity.binding.banner.start()
                        mActivity.binding.banner.setLoopTime(mActivity.loopTime)
                    }
                }
            })
        }
    ...
    internal class VideoHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val video: MyVideoPlayer = itemView.findViewById(R.id.banner_vp)
        val title: TextView = itemView.findViewById(R.id.video_title)
        val numIndicator:TextView = itemView.findViewById(R.id.video_numIndicator)
    }
    ...
    internal class ImageHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val numIndicator:TextView = itemView.findViewById(R.id.image_numIndicator)
    }
