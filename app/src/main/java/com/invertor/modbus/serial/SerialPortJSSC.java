...
public class SerialPortJSSC extends SerialPort {
    ...
    final private com.example.reallin.buyapp.jssc.SerialPort port;

    public SerialPortJSSC(SerialParameters sp) throws SerialPortException {
        super(sp);
        this.port = new com.example.reallin.buyapp.jssc.SerialPort(sp.getDevice());
    }
    ...
    public void open() throws SerialPortException {
        try {
            ...
            port.setFlowControlMode(com.example.reallin.buyapp.jssc.SerialPort.FLOWCONTROL_NONE);
        } catch (Exception ex) {
            throw new SerialPortException(ex);
        }
    }
    //emulator @chuan -writable-system -qemu -serial COM3
