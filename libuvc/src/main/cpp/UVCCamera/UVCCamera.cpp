...
int UVCCamera::setImageObjectCallback(JNIEnv *env, jobject object_callback_obj) {
    ENTER();
    int result = EXIT_FAILURE;
    if (mPreview) {
        result = mPreview->setImageObjectCallback(env, object_callback_obj);
    }
    RETURN(result, int);
}
...
void UVCCamera::drawFps(bool drawFps) {
    ENTER();
    if (mPreview) {
        mPreview->imageObjectCallback.drawFps = drawFps;
    }
}

void UVCCamera::useYolo(bool useYolo) {
    ENTER();
    if (mPreview) {
        mPreview->imageObjectCallback.useYolo = useYolo;
    }
}