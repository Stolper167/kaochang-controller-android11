...
int UVCPreview::setImageObjectCallback(JNIEnv *env, jobject object_callback_obj) {
    ENTER();
    pthread_mutex_lock(&capture_mutex);
    {
        if (isRunning() && isCapturing()) {
            mIsCapturing = false;
            if (imageObjectCallback.object) {
                pthread_cond_signal(&capture_sync);
                pthread_cond_wait(&capture_sync, &capture_mutex);	// wait finishing capturing
            }
        }
        //
        if(object_callback_obj) {
            if (!imageObjectCallback.object || !env->IsSameObject(imageObjectCallback.object, object_callback_obj))	{
                imageObjectCallback.onData = nullptr;
                if (imageObjectCallback.object) {
                    env->DeleteGlobalRef(imageObjectCallback.object);
                    imageObjectCallback.object = nullptr;
                }
                //
                imageObjectCallback.object = object_callback_obj;
                // get method IDs of Java object for callback
                jclass clazz = env->GetObjectClass(object_callback_obj);
                if (LIKELY(clazz)) {
                    imageObjectCallback.onData = env->GetMethodID(clazz,
                                                                  "onData", "(Ljava/util/List;)V");
                    // env
                    if(!imageObjectCallback.env) {
                        imageObjectCallback.env = env;
                    }
                    if (!imageObjectCallback.clz) {
                        jclass clz = env->FindClass("com/hwcas/libyolov8/ImageObject");
                        imageObjectCallback.clz = (jclass) env->NewGlobalRef(clz);
                    }
                } else {
                    LOGW("failed to get object class");
                }
                env->ExceptionClear();
                if (!imageObjectCallback.onData) {
                    LOGE("Can't find IFrameCallback#onFrame");
                    env->DeleteGlobalRef(object_callback_obj);
                    imageObjectCallback.object = object_callback_obj = NULL;
                }
            } else {
                env->DeleteGlobalRef(imageObjectCallback.object);
                imageObjectCallback.object = nullptr;
		...
		if (frameMode) {
			...
			for ( ; LIKELY(isRunning()) ; ) {
				...
				if (LIKELY(frame_mjpeg)) {
					...
					if (LIKELY(!result)) {
						frame = draw_preview_one(env, frame, &mPreviewWindow, uvc_any2rgbx, 4);
...
int copyToSurface(JNIEnv *env,
                  uvc_frame_t *frame,
                  ANativeWindow **window,
                  Image_object_callback& callback) {
	// ENTER();
	int result = 0;
	if (LIKELY(*window)) {
		ANativeWindow_Buffer buffer;
		if (LIKELY(ANativeWindow_lock(*window, &buffer, NULL) == 0)) {
            // 检测
//            if(callback.drawFps || callback.useYolo) {
//                detection(env, frame, callback);
//            }
			// source = frame data
...
uvc_frame_t *UVCPreview::draw_preview_one(JNIEnv *env, uvc_frame_t *frame, ANativeWindow **window, convFunc_t convert_func, int pixcelBytes) {
	// ENTER();
	...
	if (LIKELY(b)) {
		...
		if (convert_func) {
			...
			if LIKELY(converted) {
				...
				if (!b) {
                    pthread_mutex_lock(&preview_mutex);
                    copyToSurface(env, converted, window, imageObjectCallback);
					pthread_mutex_unlock(&preview_mutex);
				} else {
			...
			pthread_mutex_lock(&preview_mutex);
			copyToSurface(env, frame, window, imageObjectCallback);
			pthread_mutex_unlock(&preview_mutex);
		}
	}
		...
		if (LIKELY(frame)) {
			...
			if LIKELY(isCapturing()) {
				...
				if (LIKELY(converted)) {
					...
					if (!b) {
						if (LIKELY(mCaptureWindow)) {
							copyToSurface(env, converted, &mCaptureWindow, imageObjectCallback);
						}
					}
				}
			}
