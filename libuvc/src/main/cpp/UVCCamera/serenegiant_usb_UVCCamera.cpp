...
static jint nativeSetImageObjectCallback(JNIEnv *env, jobject thiz,
                                   ID_TYPE id_camera, jobject jIImageObjectCallback) {

    jint result = JNI_ERR;
    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        jobject object_callback_obj = env->NewGlobalRef(jIImageObjectCallback);
        result = camera->setImageObjectCallback(env, object_callback_obj);
    }
    RETURN(result, jint);
}
...
static void nativeDrawFps(JNIEnv *env, jobject thiz,
                          ID_TYPE id_camera, jboolean drawFps) {
    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        camera->drawFps(drawFps);
    }
}

static void nativeUseYolo(JNIEnv *env, jobject thiz,
                          ID_TYPE id_camera, jboolean useYolo) {
    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        camera->useYolo(useYolo);
    }
}
...
//	{ "nativeSetImageObjectCallback",			"(JLcom/jiangdg/uvc/IImageObjectCallback;)I", (void *) nativeSetImageObjectCallback },
...
//	{ "nativeGetFocusSimple",			"(J)I", (void *) nativeGetFocusSimple },
	...
	{ "nativeDrawFps",				"(JZ)V", (void *) nativeDrawFps },
	{ "nativeUseYolo",				"(JZ)V", (void *) nativeUseYolo },
};
